...
\chapter{Slutsatser}
Rapporten syftade på att framlägga målet med projektet samt beskriva arbetsmetoder och verktyg som har använts under projektets gång. I rapporten har det även reflekterats över projektets metodval och gett en förståelse för hur valda systemutvecklingsmetoder har fungerat i en verklig miljö.
...
Utvecklingsteamet beslutade att använda Kinect för Xbox360 som är första generationens Kinect då det var det som fanns tillgängligt från projeketets start. Det ansågs sedan inte tillräckligt prioriterat att byta till Kinect One vilket är andra generationens Kinect eftersom prestandaskillnaden mellan dessa två kameror inte motiverade de ändringarna som skulle behöva göras i kodbasen. Kinect-kamerorna används för att läsa in punktdata med hjälp av den inbyggda IR-kameran. Punktdatan kan sedan användas för att skapa en polygonmodell. Genom att använda flera Kinect-kameror erhålls tillräckligt med täckning för att hela kroppen ska bli scannad och inga inställningar behöver utföras under processen. Processen har gjorts helt automatiserad för att förenkla användningen och användaren kan starta scanningen genom ett enkelt webbaserat gränssnitt. Därmed har en 3D-modell av god kvalité kunnat skapas utan vidare invecklade inställningar och instruktioner.
...
Framtida förbättringar av systemet som kan utvecklas är att koppla en 3D-skrivare till systemet för en direkt utskrift av modellen i samband med scanningen. Eftersom den färdiga modellen sparas som en standardiserad \emph{obj-fil}, är 3D-utskrift redan idag en möjlighet. 
...
\item Johan Reimann: Gruppledare, har arbetat med scanning(kalibrering av IR- och färgkamera), texturering och hantering av filer.
...
\newpage
\chapter{Användartest}\label{app:gantt}
\begin{figure}[h]
